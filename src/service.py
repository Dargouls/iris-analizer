...
from .utils.formatRelatorio import parse_relatorio
...
class AnalisadorIridologicoNLP:
  ...
  def gerar_interpretacao(self, metricas):
      ...
      if 'iris' in metricas:
          ...
          interpretacao.append(f"Iris: {self.referencias['iris']['densidade'][densidade]}")
...
def processar_imagem(imagem):
      try:
          ...
          relatorio += f"Iris: Centro ({ix}, {iy}), Raio {ir}px\n"
...
def encode_image_to_base64(image: np.ndarray) -> str:
    """
    Codifica uma imagem numpy em base64.
    """
...
async def analisar_iris(file: UploadFile = File(...)):
    try:
        ...
        print('imagem lida')
        # Processar imagem
        img_processada, relatorio = processar_imagem(imagem)
        print('Processamento concluído')
        
        # Codificar imagem processada em base64
        img_base64 = encode_image_to_base64(img_processada)
        print('Imagem codificada base64')
        
        resultado_estruturado = parse_relatorio(relatorio)
        
        # Responder com JSON
        return JSONResponse(content={
            "imagem_processada": img_base64,
            "relatorio": resultado_estruturado
        })
